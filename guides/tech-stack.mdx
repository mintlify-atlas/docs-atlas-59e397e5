...
<Columns cols={2}>
  ...
  <Card title="@react-three/drei 9" icon="star">
    Helper library for react-three/fiber. Provides `Environment`, `PresentationControls`, `Html`, `useGLTF`, and `useProgress`.
  </Card>
  ...
  <Card title="react-responsive" icon="mobile">
    Responsive breakpoint hook. Used to conditionally render the mobile or desktop loader image at the `≤768px` breakpoint.
  </Card>
</Columns>
...
- `<Canvas>` in `src/models/Computer.jsx` (or the parent that renders it) sets up the WebGL context.
...
| `useProgress` | `src/components/Loader.jsx` | Reports GLTF load progress (0–100) to drive the loading bar. |
...
// Example: zoom into the monitor when clicked
...
| `react-responsive` | ^10.0.0 | Currently used for the loader image breakpoint; can be expanded for fully responsive 3D layouts |
