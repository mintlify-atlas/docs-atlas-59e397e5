...
The environment changes based on the time of day. The `getPeriodOfDay` function in `src/models/Computer.jsx` maps hour ranges to environment preset names.
...
<select onChange={handleEnvironmentChange}>
  ...
  <option value="warehouse">Afternoon</option>
  <option value="dawn">Evening</option>
